...
export class CreateCarSchedulingService {
  ...
  async execute(car_scheduling: ICreateCarSchedulingDTO): Promise<CarScheduling>{
    ...
    if(foundCarSchedulingByDriver.length > 0){
      const carScheduling = foundCarSchedulingByDriver.find(carScheduling => carScheduling.end_date_of_use === null || carScheduling.end_date_of_use === undefined);
    ...
    if(foundCarSchedulingByCar.length > 0){
      const carScheduling = foundCarSchedulingByCar.find(carScheduling => carScheduling.end_date_of_use === null || carScheduling.end_date_of_use === undefined);
